    ...
    return (
        <div>
            ...
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}> 
                <TextField variant="outlined" size="small" type="text" label="mobile" value={mobile} onChange={handleMobileChange} />
                { formErrors.mobile && <Typography style={{color : 'red'}}> {formErrors.mobile} </Typography> }
                </Grid>

                <Grid item xs={12}> 
                <TextField variant="outlined" size="small"  type="email" label="email" value={email} onChange={handleEmailChange} />
                { formErrors.email && <Typography style={{color : 'red'}}> {formErrors.email} </Typography> }
                </Grid>
                
                <Grid item xs={12}> 
                <Button type="submit" size="small" variant="contained" color="primary"> {customerName ? 'Save' : 'Add'} </Button>
                </Grid>
            </Grid>
